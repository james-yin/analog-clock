...
function draw() {
  ...
  function face() {
    ...
    for (let i = 0; i < 60; i++) {
      ...
      if (i % 5 === 0)
        (r -= l), (l *= 2), (ctx.strokeStyle = 'rgba(0, 0, 255, 0.5)');
    ...
    // am/pm
    ctx.fillText(hours < 12 ? 'AM' : 'PM', 150, 75);
...
function fillAlarmDropdowns() {
  const aHour = document.getElementById('a-hour');
  for (let i = 1; i <= 12; i++) {
    let opt = document.createElement('option');
    opt.value = i;
    opt.text = i;
    aHour.appendChild(opt);
  }
  const aMinute = document.getElementById('a-minute');
  for (let i = 0; i < 60; i++) {
    let opt = document.createElement('option');
    opt.value = i;
    opt.text = i;
    aMinute.appendChild(opt);
  }
...
function setAlarm() {
  let aHour = document.getElementById('a-hour').value;
  const aMinute = document.getElementById('a-minute').value;
  const aSecond = document.getElementById('a-second').value;
  ...
  if (!aHour) {
    ...
    alert('Please select a valid Second.');
    return;
  }

  let isAm = document.getElementById('am').checked;
  alarmTime = new Date();
  alarmTime.setHours(+aHour + (isAm ? 0 : 12));
  alarmTime.setMinutes(+aMinute);
  alarmTime.setSeconds(+aSecond);
}
